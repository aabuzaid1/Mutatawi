...
export default function ImpactCard({ title, value, icon: Icon, trend, color }: ImpactCardProps) {
    ...
    return (
        <motion.div
            ...
            className="bg-white rounded-2xl p-4 sm:p-6 shadow-soft border border-slate-100 transition-all duration-300"
        >
            <div className="flex items-start justify-between gap-2">
                <div className="min-w-0 flex-1">
                    ...
                    {trend && (
                        <p className={cn('text-xs sm:text-sm font-medium mt-1 sm:mt-2', trend.positive ? 'text-success-600' : 'text-danger-500')}>
                            {trend.positive ? '↑' : '↓'} {Math.abs(trend.value)}%{' '}
                            <span className="text-slate-400 hidden sm:inline">هذا الشهر</span>
                        </p>
                    )}
                </div>
